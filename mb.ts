...
const cus = new Customer(fName,lName,33*i,"male",num, 1000+i)
         ...
         if (account) {
            let name = myBank.customer.find(
                ...
                console.log(`Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic
                    (name?.lastName)}Your Account Balance is ${chalk.bold.blueBright(`$ ${account.balance}`
        )}`
                );
    }
}
        ...
        let res = await inquirer.prompt({
         ...
         if (account){
            ...
            if(ans.rupee > account.balance){
                console.log(chalk.red.bold("Mojauda Balance Maskaafi Hai..."))
            }
